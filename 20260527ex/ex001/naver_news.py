...
import urllib.parse       # " " → %20  한글 같은 특수 문자 → UTF-8 기반 %XX 형태로 변환. / 는 기본적으로 인코딩되지 않음
import datetime           # parse 해석하다의 의미임. urllib.parse.quote(srcText) 한글로된 문자열이 깨지지 않게 인코딩해줌 
import json
...
def getRequestUrl(url):
    ...
    try:                                        
        ...
        rawData = response.read()    # response.read()는 서버가 준 데이터를 딱 한 번만 읽을 수 있는 스트림(Stream) 방식 변수에 담아서 활용
        responseData = rawData.decode('utf-8')

        return responseData
                             
    except Exception as e:                # 발생한 에러 객체를 e라는 이름의 변수에 담기(출력용)
...
def main():         # 프로그램의 진짜 시작점이며, 위에서 만든 함수들을 조립해 반복 제어
    ...
    while jsonResponse != None and jsonResponse['display'] != 0 and cnt < 300:    # 범위를 제한하고 싶을 때는 cnt
        for post in jsonResponse['items']:
            cnt += 1
            getPostData(post, jsonResult, cnt)    #for문으로 하나씩 돌리면서 정제 가공한다고 보면 됨 

        nextStart = jsonResponse['start'] + jsonResponse['display']
        if nextStart > 1000 or nextStart > jsonResponse['total']:
            break

        jsonResponse = getNaverSearch(node, srcText, nextStart , 100)
  
    # 파일로 저장
    with open(f'{srcText}_naver_{node}.json', 'w', encoding='utf8') as f:
        jsonFile = json.dumps(jsonResult, indent=4, sort_keys=True,  ensure_ascii=False)
        f.write(jsonFile)              #들여쓰기 4칸 #딕셔너리 순서대로 # 한글코드 안 깨지게
