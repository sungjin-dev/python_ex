...
# result = (num1 < num3) and (num2 < num3)   # T and T

# print(f'Result: {result}')        # So, T

# result = (num1 > num3) and (num2 < num3)   # F and T

# print(f'Result: {result}')       # F

# result = (num1 > num3) and (num2 > num3)   # F and F

# print(f'Result: {result}')       # F

# num1 = 10; num2 = 20; num3 = 30
# result = (num1 < num2) and (num2 < num3) and (num3 > num1)
#                #앞에 두덩이 먼저하고 순차적으로 계산
...
# # or연산자는 피연산자 중 하나라도 True가 있다면 결과값은 
...
#    # L 이미 선행단계에서 True값이 이미 나왔지 때문에  abc defined 오류가 생기지 않음. 
...
# result = '탑승 가능' if 120<= height < 170 else '탑승 불가능' 
# print(f'Result: {result}')


# height = int(input('어린이의 신장을 입력하세요.'))
# # result = (height >= 120 and height < 170)
# result = ('탑승 가능'if 120 <= height  else'탑승 불가능')
# print(f'result: {result}')
...
# result = '흑자' if incoming > outgoing else '적자'   # 배치 방식은 절대 까먹지 말자 
# print(f'Result: {result}')
...
currentLight = int(input('조도 입력: '))
targetLight = int(input('조도 입력: '))

result = '조명 ON'if currentLight < targetLight else '조명 OFF'
